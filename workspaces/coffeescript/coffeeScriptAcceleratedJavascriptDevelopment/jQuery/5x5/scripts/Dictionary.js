...
Dictionary = (function() {
  ...
  Dictionary.prototype.setGrid = function(grid) {
    ...
    if (!(this.grid instanceof Grid)) {
      throw new Error("Not an instance of a Grid");
    }
  ...
  Dictionary.prototype.isNewWord = function(word) {
    return __indexOf.call(this.usedWords, word) < 0 && __indexOf.call(this.wordList, word) >= 0;
  };
    ...
    for (length = _i = _ref = this.minWordLength, _ref1 = grid.size; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; length = _ref <= _ref1 ? ++_i : --_i) {
      ...
      for (offset = _j = 0; 0 <= length ? _j < length : _j > length; offset = 0 <= length ? ++_j : --_j) {
        ...
        if (grid.inRange(x - offset, y - offset) && grid.inRange(x - offset + range, y - offset + range)) {
          addTiles(function(j) {
            return grid.tiles[x - offset + j][y - offset + j];
          });
        }
        if (grid.inRange(x - offset, y + offset) && grid.inRange(x - offset + range, y + offset - range)) {
          addTiles(function(k) {
            return grid.tiles[x - offset + k][y + offset - k];
          });
        }
      }
    }
